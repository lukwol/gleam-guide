...
Create `server/.envrc` to handle this:
...
[direnv](https://direnv.net) loads `.envrc` automatically when you enter the directory. It first loads the root `.env` with `dotenv`, then overrides `PGHOST` with `localhost` — so Squirrel can reach the database through the mapped port without touching the shared `.env` file.
...
The query functions are ready but they need a `pog.Connection` to run against. The next step is loading the server configuration from environment variables and setting up a supervised database connection pool to pass through the router.
