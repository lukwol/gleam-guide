...
- **`UnexpectedNoRows` vs `RecordNotFound`** — `create_task` uses `UnexpectedNoRows` because `RETURNING` guarantees a row on success; an empty result is a bug. `get_task` and `update_task` use `RecordNotFound` because an empty result simply means the record doesn't exist.
